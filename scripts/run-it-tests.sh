...
export DEDER_TMP_M2_REPO=$(realpath .deder/tmp-m2)
...
# Copy plugin-api artifacts into the shared tmp-m2 repo so hello-plugin can resolve it
mkdir -p "$DEDER_TMP_M2_REPO/ba/sake"
cp -r ~/.m2/repository/ba/sake/deder-plugin-api_3 "$DEDER_TMP_M2_REPO/ba/sake/"
